--- Diff module for Codex Code Neovim integration.
...
local function get_autocmd_group()
  if not autocmd_group then
    autocmd_group = vim.api.nvim_create_augroup("CodexMCPDiff", { clear = true })
  end
...
---Find the Codex Code terminal window to keep focus there.
---Uses the terminal provider to get the active terminal buffer, then finds its window.
---@return number? win_id Window ID of the Codex Code terminal window, or nil if not found
...
---Display existing Codex Code terminal in new tab
...
local function display_terminal_in_new_tab()
  ...
  local existing_terminal_win = find_codex_terminal_window()
  ...
  -- Optionally hide the Codex terminal in the new tab for more review space
  ...
  vim.api.nvim_create_autocmd("BufEnter", {
    ...
    desc = "Auto-enter terminal mode when focusing Codex Code terminal",
  })
...
---@param user_config CodexConfig The configuration passed from init.lua
...
local function create_temp_file(content, filename)
  local base_dir_cache = vim.fn.stdpath("cache") .. "/codex_diffs"
  ...
  if mkdir_ok_cache then
    ...
    local base_dir_temp = vim.fn.stdpath("cache") .. "/codex_diffs_fallback"
...
local function cleanup_temp_file(tmp_file)
  if tmp_file and vim.fn.filereadable(tmp_file) == 1 then
    ...
    if vim.fs and type(vim.fs.remove) == "function" then
      ...
      if not ok_file then
        vim.notify(
          "Codex: Error removing temp file " .. tmp_file .. ": " .. tostring(err_file),
          vim.log.levels.WARN
        )
      end
      ...
      if not ok_dir then
        vim.notify(
          "Codex: Error removing temp directory " .. tmp_dir .. ": " .. tostring(err_dir),
          vim.log.levels.INFO
        )
      end
    else
      ...
      vim.notify(
        "Codex: Cannot perform standard cleanup: "
          .. reason
          .. ". Affected file: "
          .. tmp_file
      ...
      if not os_ok then
        vim.notify(
          "Codex: Fallback os.remove also failed for file " .. tmp_file .. ": " .. tostring(os_err),
          vim.log.levels.ERROR
        )
      end
    end
...
local function setup_new_buffer(
  ...
  vim.b[new_buf].codex_diff_tab_name = tab_name
  vim.b[new_buf].codex_diff_new_win = new_win
  vim.b[new_buf].codex_diff_target_win = target_win_for_meta
...
function M._open_native_diff(old_file_path, new_file_path, new_file_contents, tab_name)
  ...
  local cleanup_group = vim.api.nvim_create_augroup("CodexDiffCleanup", { clear = false })
...
function M._cleanup_diff_state(tab_name, reason)
  ...
  if diff_data.created_new_tab then
    ...
    -- Optionally ensure the Codex terminal remains visible in the original tab
    local terminal_ok, terminal_module = pcall(require, "codex.terminal")
...
function M.close_diff_by_tab_name(tab_name)
  ...
  if diff_data.status == "saved" then
    -- Codex Code CLI has written the file, reload any open buffers
    if diff_data.old_file_path then
      -- Add a small delay to ensure Codex CLI has finished writing the file
...
function M.accept_current_diff()
  ...
  local tab_name = vim.b[current_buffer].codex_diff_tab_name
