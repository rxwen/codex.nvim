---Manages selection tracking and communication with the Codex server.
...
function M.update_selection()
  ...
  -- If the buffer name starts with "✻ [Codex Code]", do not update selection
  if buf_name and string.sub(buf_name, 1, string.len("✻ [Codex Code]")) == "✻ [Codex Code]" then
...
function M.handle_selection_demotion(original_bufnr_when_scheduled)
  ...
  -- Condition 1: Switched to Codex Terminal
  if claude_term_bufnr and current_buf == claude_term_bufnr then
  ...
  -- Condition 3: Still in Original Buffer & Not Visual & Not Codex Term -> Demote
...
---Sends the selection update to the Codex server.
...
---Sends the current selection to Codex.
...
function M.send_current_selection()
  if not M.state.tracking_enabled or not M.server then
    logger.error("selection", "Codex Code is not running")
    return
  end
...
function M.send_at_mention_for_visual_selection(line1, line2)
  ...
  -- Check if Codex Code integration is running (server may or may not have clients)
  local codex_main = require("codex")
  if not codex_main.state.server then
    logger.error("selection", "Codex Code integration is not running.")
    return false
  end
  ...
  local success, error_msg = codex_main.send_at_mention(file_path, start_line, end_line, "CodexSend")
