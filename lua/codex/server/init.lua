---@brief WebSocket server for Codex Code Neovim integration
local codex_main = require("codex") -- Added for version access
local logger = require("codex.logger")
local tcp_server = require("codex.server.tcp")
local tools = require("codex.tools.init") -- Added: Require the tools module
...
function M.start(config, auth_token)
  ...
  local callbacks = {
    ...
    on_connect = function(client)
      ...
      local main_module = require("codex")
  ...
  M.state.handlers = {
    ["initialize"] = function(client, params)
      return {
        ...
        serverInfo = {
          name = "codex-neovim",
          version = codex_main.version:string(),
        },
      }
    end,
