-- Tool implementation for Codex Code Neovim integration
local M = {}
...
function M.register_all()
  ...
  M.register(require("codex.tools.open_file"))
  M.register(require("codex.tools.get_current_selection"))
  M.register(require("codex.tools.get_open_editors"))
  M.register(require("codex.tools.open_diff"))
  M.register(require("codex.tools.get_latest_selection"))
  M.register(require("codex.tools.close_all_diff_tabs"))
  M.register(require("codex.tools.get_diagnostics"))
  M.register(require("codex.tools.get_workspace_folders"))
  M.register(require("codex.tools.check_document_dirty"))
  M.register(require("codex.tools.save_document"))

  -- Register internal tools without schemas (not exposed via MCP)
  M.register(require("codex.tools.close_tab"))
end
...
function M.handle_invoke(client, params) -- client needed for blocking tools
  ...
  if tool_data.requires_coroutine then
    ...
    require("codex.logger").debug("tools", "Wrapping " .. tool_name .. " in coroutine for blocking behavior")
    local co = coroutine.create(function()
      return tool_data.handler(input)
    end)

    require("codex.logger").debug("tools", "About to resume coroutine for " .. tool_name)
    local success, result = coroutine.resume(co)
    require("codex.logger").debug(
      "tools",
    ...
    if coroutine.status(co) == "suspended" then
      require("codex.logger").debug("tools", "Coroutine is suspended - tool is blocking, will respond later")
